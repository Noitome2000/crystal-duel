(function(root){
  ...
  function create(mode='classic'){return {mode,stage:'coin',first:null,index:0,picks:{red:[],blue:[]},history:[]};}
  ...
  function choose(s,side,hero){const turn=current(s),slots=s.mode==='battle'?3:2;if(!turn||turn.side!==side)throw Error('请按顺序选将');if(!R.HEROES[hero]||s.picks[side].includes(hero))throw Error('请选择未使用的英雄');s.picks[side].push(hero);s.history.push({...turn,hero});s.index++;if(s.index===slots*2)s.stage='ready';return current(s);}
