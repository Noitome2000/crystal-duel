...
(function(root){
  ...
  function key(s){return `${s.side}/${s.phase}/${s.actor}/${s.combo}/${s.noDodge}/${Math.min(s.ply,6)}/`+s.units.filter(u=>u.alive).map(u=>`${u.id}:${u.x},${u.y},${!!u.charged},${Math.max(0,(u.pushImmuneUntil||0)-s.ply)},${Math.max(0,(u.swapImmuneUntil||0)-s.ply)},${Math.max(0,(s.cooldowns[u.id]||0)-s.ply)}`).join(';');}
  function evaluate(s,side){
    ...
    for(const team of ['red','blue'])for(const id of threatened[team])score+=(team===side?1:-1)*value(R.get(s,id))*.23;
    return score;
  }
  async function choose(s,{side=owner(s),maxDepth=3,maxNodes=2400,timeMs=550,history=[],cancelled=()=>false}={}){
    ...
    const assess=n=>evaluate(n,side)-(history.filter(k=>k===key(n)).length*14);
    async function ranked(n,rootNode){
      ...
      return list.slice(0,rootNode?16:8);
    }
    async function search(n,depth,alpha,beta,extensions){
      await checkpoint();if(n.winner)return evaluate(n,side);
      if(depth<=0){if((n.phase==='normal'&&!n.actor)||extensions<=0)return assess(n);depth=1;extensions--;}
    ...
    return {action:cancelled()?null:best,stats:{nodes,depth:completedDepth,score,elapsedMs:Date.now()-start}};
  }
