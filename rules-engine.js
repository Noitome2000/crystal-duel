...
(function (root) {
  ...
  const HEROES = {
    general:{name:'将军',skill:'推进 / 军号',desc:'推进：推开相邻敌人一格并占据其位置，敌人下回合不能反推。军号：周围士兵共享推进与冷却。'},
    strategist:{name:'军师',skill:'设陷',desc:'设陷：封锁横纵两格外的敌人，使其不能移动、攻击、瞬闪或游击；跳跃技能仍可用。'},
    vanguard:{name:'先锋',skill:'陷阵 / 斩将',desc:'陷阵：跃过相邻棋子，落在其后空格，开局可用。斩将：击破英雄也能连击，连击可攻击英雄。'},
    assassin:{name:'刺客',skill:'瞬移 / 短兵',desc:'瞬移：跳至任意空格。短兵：攻击距离固定为一格。'},
    ranger:{name:'游侠',skill:'瞬闪 / 游击',desc:'瞬闪：受击前移至相邻空格，攻击者从原落点再行动一次，期间不能再次瞬闪。游击：攻击结束后可移动一格。'},
    pikeman:{name:'尖兵',skill:'武库',desc:'武库：首次攻击可走一格或两格，连击走满两格。'},
    mage:{name:'魔术师',skill:'移形',desc:'移形：交换身边九宫格内的两个相邻棋子。换位保护持续到被换位者下回合结束。'},
    dragon:{name:'巨龙',skill:'轰炸',desc:'轰炸：先蓄力，下个己方回合沿横纵跳至三格内（可原地），摧毁落点及周围所有其他棋子。'},
    knight:{name:'骑士',skill:'神速 / 荣耀',desc:'神速：连续移动两格，不穿人，开局可用。荣耀：周围士兵共享神速与冷却。'},
    wolf:{name:'狼灭',skill:'残忍 / 斩将',desc:'残忍：可攻击己方棋子。斩将：击破英雄也能连击，连击可攻击英雄。'}
  };
  const CELLS=[];
