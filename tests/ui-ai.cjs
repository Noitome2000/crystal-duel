...
(async()=>{
 ...
 await page.locator('[data-action="move"] .sector-label').click();assert.ok(await page.locator('#actionWheel').evaluate(e=>e.classList.contains('collapsed')));
 ...
 await page.click('[data-unit="r"]');await page.locator('[data-action="move"] .sector-label').click();
