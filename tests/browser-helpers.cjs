...
async function draftGame(page,{mode='local',first='red',picks={red:['general','knight'],blue:['vanguard','assassin']}}={}){
  ...
  for(let i=0;i<4;i++){
    ...
    if(mode==='local'||side==='red'){await page.locator(`[data-hero="${picks[side][Math.floor(i/2)]}"]`).click();if(await page.locator('#confirmHero').isVisible())await page.locator('#confirmHero').click();}
