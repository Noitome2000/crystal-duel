...
(async()=>{
  ...
  await page.locator('[data-action="move"] .sector-label').click();
  ...
  for(let i=0;i<28;i++){
    ...
    if(!choice)break;
    await page.click(`[data-unit="${choice.id}"]`);await page.locator(`[data-action="${choice.mode}"] .sector-label`).click();
